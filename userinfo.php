<?php
	...
	if(isset($_GET['userfullname'])){
		...
		// escape sql chars
		$userfullname = mysqli_real_escape_string($conn, $_GET['userfullname']);

		// make sql
		$sql = "SELECT * FROM users WHERE userfullname = $userfullname";
...
<html>
  ...
  <style type="text/css">
	...
	<div class="container center">
		<?php if($viewid): ?>
			<h4><?php echo $viewid['userid']; ?></h4>
			<p><?php echo $viewid['userfullname']; ?></p>
			<p><?php echo $viewid['useremail']; ?></p>
			
		<?php else: ?>
			<h5>No such record exists.</h5>
		<?php endif ?>
	</div>
